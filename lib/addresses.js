...
AddressController.prototype.checkAddr = function(req, res, next) {
  req.addr = BchAddr.isCashAddress(req.params.addr) ? BchAddr.toLegacyAddress(req.params.addr) : req.params.addr;
...
AddressController.prototype.checkAddrs = function(req, res, next) {
  ...
  req.addrs = req.addrs.map(
    function(addr) {
      return BchAddr.isCashAddress(addr) ? BchAddr.toLegacyAddress(addr) : addr;
    }
  )
